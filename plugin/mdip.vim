...
function! s:SaveFileTMPWin32(imgdir, tmpname) abort
    ...
    let clip_command .= "$fileDropList = [System.Windows.Forms.Clipboard]::GetData([System.Windows.Forms.DataFormats]::FileDrop);"
    let clip_command .= "if ($fileDropList.Length -gt 0) { "
    let clip_command .= "$fileName = $fileDropList[0].ToLower(); if ($fileName.Contains('.png') -or $fileName.Contains('.jpg') -or $fileName.Contains('.jepg') -or $fileName.Contains('.git')) { "
    let clip_command .= "[System.IO.File]::Copy($fileName, '".tmpfile."', [System.Boolean]::True);"
    let clip_command .= "}}"
